...
int main()
{
	...
	*/
    ...
    uint8_t data[96];
    uint8_t buf[96];
    uint8_t key_exp[32] = { 0 };
    char phrase[10] = { 0 };
    strcpy(phrase, "any");
    printf("phrase = \"%s\"\n", phrase);

    md5_hash(phrase, key_exp, 32);
    aes_key_expansion(key_exp, w);

    printf("key:\n");
    print_hex(key_exp, 32);
    //print_hex(w, 240);

    strcpy((char*)data, "dfuyg832g g872387g387gg7g;[a][pDKJFIJPIJ MXBNH =k45g34j234jbeiubwebfibiuhg213864593267g_____111");

    size_t length = strlen((char*)data);
    aes_encode(data, buf, length, w);

    printf("DATA length = %zu\n%s\n", length, data);
    print_hex(buf, length);

    aes_decode(buf, data, length, w);
    printf("%s\n\n", (char*)data);
